...
class Cookie implements ICookie {
  ...
  constructor() {
    const { expires, path, domain, secure, sameSite } = environment.COOKIE_OPTIONS[environment.MODE ?? 'development'];
  ...
  setCookie(
    ...
    let { expires: expireTimes, path, domain, secure, sameSite } = options || {}
    if (!keyName) {
  ...
  removeCookie(keyName: keyName, options?: RemoveCookieOptions) {
  ...
  isCookieAvailable(keyName: keyName) {
    return new RegExp(
      '(?:^|;\\s*)' +
      encodeURIComponent(keyName).replace(/[\-\.\+\*]/g, '\\$&') +
      '\\s*\\='
